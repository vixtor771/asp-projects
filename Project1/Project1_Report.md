...
In this project I measured my own hearing threshold and four equal-loudness curves with a listening test written in Python, compared them with the ISO 226 curves, and built an equalizer that takes a curve as its parameter. Then I listened to what it does to speech, music, noise and a sweep. The code is in the submitted Project1 folder (Python 3.14). `python main.py` shows a menu with three options: the listening test, the equalizer on the test sounds, and a 10 s microphone recording that gets equalized. 
...
Figure 1 shows the five measured curves in dBFS, with ISO 226 contours drawn dashed for comparison. Each dashed contour is shifted to pass through my 1 kHz point; its phon value (11, 23, 36 or 51) follows from assuming that my 1 kHz threshold equals the ISO threshold of 2.4 dB SPL. The values are in `figures/elc_measured_table.md`.
...
- Below 1 kHz my curves follow the dashed ISO contours; the two quietest ones need about 10 dB more at 100 Hz than the standard says. Above 2.5 kHz the ISO contours rise again toward 8 to 10 kHz, but mine keep falling, and at 10 kHz they sit 20 to 30 dB below the standard. Since all five curves do this, it is most likely the headphones (see Section 6).
...
- Pink noise: my equalizer reduces the noise clearly, the hiss almost disappears, while the ISO equalizer does not. The hiss sits above 2 kHz, which my curve cuts by up to 20 dB while the ISO curve boosts it.
